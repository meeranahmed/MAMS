...
import { useNavigation } from "@react-navigation/native";
  ...
  export default function ContractAlarm() {
  ...
  useEffect(() => {
    apiServices.getContract().then(async (response) => {
      ...
      <TouchableWithoutFeedback  onPress={ () => (Press (item.id))}> 
        <View style={styles.container}>
          ...
          <Text style={styles.date}>{item.id}:</Text>
             ...
             <View>
             <Text style={styles.date}>Reamaining Days: {item.remaining_time}</Text>
             </View>
  ...
  let reverse = History.reverse()
    return (
      <View style={styles.container}>
        <Text  style={styles.title} >Devices That Their Contract Is About To End:</Text>  
